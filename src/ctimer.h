...
{
public:
    CTimerBase(float flInitialInterval, bool bPreserveMapChange) : m_flInterval(flInitialInterval), m_bPreserveMapChange(bPreserveMapChange){};
...
{
public:
    CTimer(float flInitialInterval, bool bPreserveMapChange, std::function<float()> func) : CTimerBase(flInitialInterval, bPreserveMapChange), m_func(func)
    {
