...
#define UPDATE_CONFIG_TABLE "UPDATE `cs2_rank_configs` SET `version` = '%s'";
...
#define UPDATE_USER "UPDATE `cs2_rank_users` SET `name` = '%s', `ignore_annouce` = %d, \
  `lastconnect` = %ld WHERE `authid` = '%lli' AND `reference` = '%s' AND `map` = %i;"
...
#define INSERT_STATS "INSERT INTO `cs2_rank_stats` (`user_id`, `reference`, `map` ) VALUES ('%d', '%s', '%s');"

#define UPDATE_STATS "UPDATE `cs2_rank_stats` `points` = %d, `death_suicide` = %d, `death_t` = %d, `death_ct` = %d, `bomb_planted` = %d, \
`bomb_exploded` = %d, `bomb_defused` = %d, `kill_knife` = %d, `kill_headshot` = %d, `kill_t` = %d, \
`kill_ct` = %d, `teamkill_t` = %d, `teamkill_ct` = %d, WHERE `user_id` = '%d' AND `reference` = '%s' AND `map` = %i;"

#define SELECT_STATS_MAP "SELECT `points`, `death_suicide`, `death_t`, `death_ct`, `bomb_planted`, `bomb_exploded` \
, `bomb_defused`, `kill_knife`, `kill_headshot`, `kill_t`, `kill_ct`, `teamkill_t` \
, `teamkill_ct`, `killassist_t`, `killassist_ct` FROM `cs2_rank_stats` WHERE `user_id` = '%i' AND `reference` = '%s' AND `map` = %i"

#define SELECT_STATS_GLOBAL "SELECT SUM(points) AS total_points, SUM(death_suicide) AS total_death_suicide, \
...
SUM(killassist_ct) AS total_killassist_ct FROM cs2_rank_stats WHERE user_id = '%i' and `reference` = '%s' GROUP BY user_id;"

// RANK REQUEST

//#define TOP "SUM(points) as total_points FROM cs2_rank_stats WHERE points >= %i AND `reference` = '%s' ORDER BY points DESC LIMIT 15;"
// TODO: ADD NAME FROM cs2_rank_users
#define TOP_GLOBAL "SELECT SUM(points) as total_points FROM cs2_rank_stats WHERE `reference` = '%s' GROUP BY user_id HAVING total_points >= %i \
  ORDER BY total_points DESC LIMIT 15;"

#define RANK_GLOBAL "SELECT COUNT(DISTINCT user_id) as count FROM ( SELECT user_id, SUM(points) as total_points FROM cs2_rank_stats \
    WHERE `reference` = '%s' GROUP BY user_id) AS subquery WHERE total_points > %i;"

// TODO: ADD NAME FROM cs2_rank_users
#define TOP_MAP "SELECT `points` FROM cs2_rank_stats WHERE points >= %i AND `reference` = '%s' AND `map` = %i ORDER BY points DESC LIMIT 15;"
#define RANK_MAP "SELECT COUNT(*) FROM `cs2_rank_stats` WHERE `points` > %i AND `reference` = '%s' AND `map` = %i;"
